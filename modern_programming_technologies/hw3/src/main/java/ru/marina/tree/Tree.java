...
public class Tree {
    ...
    private Node getNodeByIdRecursive(Node node, int id) {
        if (node.getId() == id)
            return node;
        for (Node n : node.getChildren()) {
            Node res = getNodeByIdRecursive(n, id);
            if (res != null) {
                return res;
            }
        }
    ...
    public Node getNodeById(Tree tree, int id) {
        return getNodeByIdRecursive(tree.getRoot(), id);
    }
}
